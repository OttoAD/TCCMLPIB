...
indexTable = readData(fileName = "./ELM/data/Dados.csv", index = "Periodo", dates = True, period = "Q")
originalGDP = readData(fileName = "./ELM/data/PIB.csv", index = "Periodo", dates = True, period = "Q")
...
##### REDE NEURAL
# media = 0 #centro da distribuição
# desvio = 1 #Desvio
# amostras = shuffledGDP.shape[0] #numero de amostras
# pesos = np.random.normal(media, desvio, amostras) #


# def sigmoid(z):
#     return 1.0/(1.0 + np.exp(-z))

# def feedforward(a,bias,weights):
#     for b, w in zip(bias, weights):
#         a = sigmoid(np.dot(w, a).sum(b)) #precisa do sum B?
#     return a #No caso, isso é a matriz H da ELM

# def beta(H,T): #Função de cálculo do beta
#     return H.T.dot(T)


# teste = feedforward(shuffledGDP.iloc[:,1:],shuffledGDP.iloc[:,0],pesos)
...
def generate_weights(data, n_neurons):
    n_features = data.shape[1]
    size = (n_features, n_neurons)
    return pd.DataFrame(np.random.normal(0, 1, size = size)) #rs.normal(size=hw_size)

opa = generate_weights(shuflledIndex,20)

print(opa)

#teste = shuflledIndex.dot(opa.T)
